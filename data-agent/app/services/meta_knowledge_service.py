...
from app.clients.embedding_client_manager import HuggingFaceEndpointEmbeddings
...
class MetaKnowledgeService:
    ...
    async def __save_tables_to_meta_db(self, meta_config: MetaConfig) -> list[ColumnInfo]:
        table_infos: list[TableInfo] = []
        column_infos: list[ColumnInfo] = []
        for table in meta_config.tables:  # table -> table_info
            ...
            for column in table.columns:  # column -> column_info
                # 查询字段取值实例
    ...
    async def __save_columns_to_qdrant(self, column_infos: list[ColumnInfo]):
    ...
    async def __save_values_to_es(self, meta_config: MetaConfig):
            ...
            for column in table.columns:
                if column.sync:
                    current_column_values = await self.dw_mysql_repository.get_column_values(table.name,
                                                                                             column.name,
                                                                                             limit=100000)
                    current_values_info = [ValueInfo(id=f'{table.name}.{column.name}.{current_column_value}',
                                                     value=current_column_value,
                                                     column_id=f'{table.name}.{column.name}') for
                                           current_column_value in current_column_values]
                    value_infos.extend(current_values_info)

        await self.value_es_repository.index(value_infos)

    async def __save_metrics_to_meta_db(self, meta_config: MetaConfig) -> list[MetricInfo]:
        metric_infos: list[MetricInfo] = []
        column_metrics: list[ColumnMetric] = []
        for metric in meta_config.metrics:  # metric -> MetricInfo
            ...
            for column in metric.relevant_columns:  # column -> ColumnMetric
    ...
    async def __save_metrics_to_qdrant(self,metric_infos: list[MetricInfo]):
    ...
    async def build(self, config_path: Path):
        ...
        # 2. 根据配置文件同步指定表信息
        if meta_config.tables:  # 配置文件是否有表信息
            # 2.1 将表信息和字段信息保存到meta数据库中(实体类->repository->session->add_all)
            column_infos = await self.__save_tables_to_meta_db(meta_config)

            # 2.2 对字段信息建立向量索引
            await self.__save_columns_to_qdrant(column_infos)

            # 2.3 对指定的维度字段取值建立全文索引
            await self.__save_values_to_es(meta_config)

        # 3. 根据配置文件同步指定指标信息
        if meta_config.metrics:  # 配置文件是否有指标信息
            # 3.1 将指标信息保存到meta数据库中
            metric_infos = await self.__save_metrics_to_meta_db(meta_config)
            # 3.2 对指标信息建立向量索引
            await self.__save_metrics_to_qdrant(metric_infos)
